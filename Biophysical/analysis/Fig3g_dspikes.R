...
############################################################
## A script to analyse the dendritic membrane potential responses of the CA1 cell
...
for (i.w  in c(1,2)){
	...
	for (i.clust in 1:7){
		...
		for (stimseed in 1:nseed){
			...
			rD <- array(dresp, dim=c(8, 50001, 16))
...
write.table(format(round(dsp_data, 6), scientific=F), file='Fig3g_data.txt')
