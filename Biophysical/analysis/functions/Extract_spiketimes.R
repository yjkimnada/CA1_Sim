...
extract.spiketimes <- function(voltage, sampling.freq, graphics=0, limite=0, dvdt.th=5, fullWidthHalfMax=F){
   ...
   # dur: of the spikes from the threshold crossing to the first point below the threshold
	...
	if (length(st) > 0){
	    ...
	    if (fullWidthHalfMax == T){
			t.spikes <- seq(dt, length=size.spike.shape, by=dt)
			tt <- t.spikes[(t.threshold/dt): (end.spike/dt)]			
			vv <- m.spike.shape[(t.threshold/dt): (end.spike/dt)]
			halfmax <- (max(vv) + vv[1]) / 2
			# plot(tt, vv, t='o')
			# abline(h=halfmax)
			
			imax <- which.max(vv)
			K <- length(vv)
			t1 <- approx(vv[1:imax], tt[1:imax], halfmax)
			t2 <- approx(vv[imax:K], tt[imax:K], halfmax)
			spike_width <- t2$y - t1$y
			# abline(v=c(t1, t2))
	    		out$width = spike_width
	    }
