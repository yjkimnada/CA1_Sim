...
# ----------------------------------------------------------
# SIMULATION RUN
def simulate(model, t_stop=100, NMDA=False, recDend=False, i_recDend=11, x_recDend=0.5, spines=False, i_recSpine=11, recGDend=False, i_recSyn=[0, 1, 2, 3,]):
    ...
    neuron.run(t_stop)
    return np.array(trec), np.array(vrec), np.array(vDendRec), np.array(gDendRec), np.array(iDendRec)#, np.array(caDendRec), np.array(vSecRec)
