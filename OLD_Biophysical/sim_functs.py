...
def genClustStarts(Nsyn, Nclust, Ncell_per_clust, seed):
    ...
    if Nsyn_rand != 0:
        ...
        clStarts = np.arange(0, Nsyn_in_clust, Ncell_per_clust)
    return clStarts
...
def sim_PlaceInput(type, Ensyn, Insyn, Erate, Irate, duration, stimseed, rep, recNMDA, elimIspike=0):
    ...
    taxis, v, vD, Gdend, Idend = lb.simulate(model, t_stop=data.TSTOP * 1000, NMDA=data.NMDA, recDend=data.recordDend, i_recDend=data.locDendRec, x_recDend=data.xDendRec, spines=data.SPINES, i_recSpine=recSpines, recGDend=False, i_recSyn=recNMDA)
